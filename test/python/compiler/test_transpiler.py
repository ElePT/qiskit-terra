...
class TestTranspile(QiskitTestCase):
    ...
    def test_already_mapped_1(self):
        ...
        backend = FakeGeneric(
            num_qubits=16, basis_gates=["cx", "id", "rz", "sx", "x"], coupling_map=cmap
        )
        coupling_map = backend.coupling_map
        basis_gates = backend.operation_names
    ...
    def test_mapping_correction(self):
        ...
        backend = FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=12)
    ...
    def test_mapping_multi_qreg(self):
        ...
        backend = FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=8)
    ...
    def test_transpile_circuits_diff_registers(self):
        ...
        backend = FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=4)
        circuits = []
        for _ in range(2):
    ...
    def test_do_not_run_gatedirection_with_symmetric_cm(self):
        ...
        layout = Layout.generate_trivial_layout(*circ.qregs)
        coupling_map = []
        for node1, node2 in FakeGeneric(
            basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=16
        ).coupling_map:
    ...
    def test_move_measurements(self):
        ...
        cmap = FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=16).coupling_map
        qasm_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "qasm")
    ...
    def test_initialize_FakeMelbourne(self):
        ...
        out = transpile(
            qc, backend=FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=4)
        )
        out_dag = circuit_to_dag(out)
    ...
    def test_multiqubit_gates_calibrations(self, opt_level):
        ...
        circ.measure_all()
        backend = FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=6)
    ...
    def test_transpile_with_custom_control_flow_target(self, opt_level):
        ...
        target = FakeGeneric(
            basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=8, control_flow=True
        ).target
...
class TestPostTranspileIntegration(QiskitTestCase):
    ...
    def test_qasm3_output_control_flow_expr(self, optimization_level):
        ...
        transpiled = transpile(
            self._control_flow_circuit(),
            backend=FakeGeneric(
                basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=27, control_flow=True
            ),
            optimization_level=optimization_level,
            seed_transpiler=2023_07_26,
        )
...
class TestTranspileParallel(QiskitTestCase):
    ...
    def test_parallel_multiprocessing(self, opt_level):
        ...
        pm = generate_preset_pass_manager(
            opt_level, backend=FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=4)
        )
        res = pm.run([qc, qc])
        for circ in res:
    ...
    def test_parallel_with_target(self, opt_level):
        ...
        qc.cx(0, 1)
        qc.measure_all()
        target = FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=4).target
...
class TestTranspileMultiChipTarget(QiskitTestCase):
    ...
    def test_transpile_does_not_affect_backend_coupling(self, opt_level):
        ...
        for i in range(1, 127):
            qc.ecr(0, i)
        backend = FakeGeneric(basis_gates=["cx", "id", "rz", "sx", "x"], num_qubits=130)
