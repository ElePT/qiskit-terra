...
.. code-block:: python
   ...
   TestSimulator().run(run_input)

Where the input is a QuantumCircuit object and the output is a TestProviderJob object, which can
...
from qiskit.providers.test_provider.test_provider_job import TestProviderJob
from .exceptions import TestProviderError
from .test_provider_tools import single_gate_matrix
from .test_provider_tools import SINGLE_QUBIT_GATES
from .test_provider_tools import cx_gate_matrix
from .test_provider_tools import einsum_vecmul_index

logger = logging.getLogger(__name__)


class TestSimulator(BackendV1):
    ...
    def _set_options(self, qobj_config=None, backend_options=None):
        ...
        if self._initial_statevector is not None:
            ...
            if round(norm, 12) != 1:
                raise TestProviderError(f"initial statevector is not normalized: norm {norm} != 1")
    ...
    def run(self, run_input, **backend_options):
        ...
        Returns:
            TestProviderJob: derived from BaseJob
        ...
        job = TestProviderJob(self, job_id, self._run_job(job_id, qobj))
        return job
    ...
    def run_experiment(self, experiment):
        ...
        Raises:
            TestProviderError: if an error occurred.
        """
        start = time.time()
            ...
            for operation in experiment.instructions:
                ...
                # Check if measure
                    ...
                    if relation == "==":
                        ...
                        raise TestProviderError("Invalid boolean function relation.")
                    ...
                    raise TestProviderError(err_msg.format(backend, operation.name))
