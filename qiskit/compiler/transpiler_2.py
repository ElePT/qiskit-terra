...
def transpile(  # pylint: disable=too-many-return-statements
    ...
    # from here on we have: target, backend.target and loose constraints
    # we need to:
    # 1. filter according to order of priorities
    # 2. construct new target to send around

    # if we have target => new_target = target
    # if we have target + loose constraints => ignore loose constraints
    # if we have target + backend => ignore backend
    # if we have backend => new_target = backend.target
    # if we have backend + loose constraints => new_target = backend.target overwritten by loose constraints
    # if we have loose constraints => new_target = Target.from_configuration()
    # if we have target + backend + loose constraints => new_target = target
